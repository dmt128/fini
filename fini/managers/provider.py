...
class ProviderBase(ABC):
    ...
    def __init__(self, settings_manager, cache_manager, webview_manager, debug=False):
        ...
        self._uctx = url_ctx(debug=debug)
        self._data = provider_data_ctx(debug=debug)
        self._opts = settings.settings_ctx(provider=self, settings_manager=settings_manager, debug=debug)
        self._dctx = pprinter.draw_ctx(debug=debug)
        self._wctx = wview.webview_ctx(provider=self, settings_manager=settings_manager, debug=debug)
    ...
    def update_draw_ctx(self, smng, key, value, old_value, user_data=None):
    ...
    def stock_data_dict_ref(self, ticker):
        return self._data_dict[ticker].data_ref()

    def stock_data_dict_copy(self, ticker):
        return self._data_dict[ticker].data_copy()
    ...
    #     spos_x = pos_x if pos_x else self._pos_x 
    #     spos_y = pos_y if pos_y else self._pos_y

    #     for panel_name, draw_cmds in self._cmd.items():
    #         panel  = self._dctx._panels[panel_name]
    #         width  = panel['width']
    #         height = panel['height']
    #         rpos_x = panel['pos_x']
    #         rpos_y = panel['pos_y']

    #         cpos_x = spos_x + rpos_x
    #         cpos_y = spos_y + rpos_y
                
    #         for cmd in draw_cmds:
    #             cmd(cpos_x, cpos_y, self._data._data)
    ...
    def __call__(self, ticker_symbol):
        ...
        self._data.load_data( cached_data )
        ...
        for URL, parser in self._uctx(ticker_symbol):
            ...
            parser(URL, website_data, self._data)

        # Increase number of records and active record entries in data
        self._data._data["num_records"] += 1
        self._data._data["active_record"] = self._data._data["num_records"] - 1

        # Save new extracted data
        self._cmng.save_stock_data( self, ticker_symbol)
    ...
    def process(self, ticker):
        ...
        # # clear it before continuing.
            # if get_size(self._data_dict) > self._smng._get_setting("global.cache_max_size"):
        ...
        # Update webviews for this provider
        for webview_data in self._wctx("update"):
            self._wmng.process(webview_data)
        ...
        for ticker_symbol in tqdm(
            ...
            if ticker_symbol not in self._data_dict:
                # Load stock data for this provider and ticker_symbol
                cached_data = self._cmng.load_stock_data( self, ticker_symbol)

                # Copy cached data to internal data dictionary
                self._data_dict[ticker_symbol] = copy.deepcopy(self._data)
                self._data_dict[ticker_symbol].load_data(cached_data)

            # Get website data, parse it and store it in internal data dictionary
            for URL, parser in self._uctx(ticker_symbol):
                website_data = util.web.get_URL_data(URL)
                parser(URL, website_data, self._data_dict[ticker_symbol])

            # Increase number of records and active record entries in data
            self._data_dict[ticker_symbol]._data["num_records"] += 1
            self._data_dict[ticker_symbol]._data["active_record"] = self._data_dict[ticker_symbol]._data["num_records"] - 1
...
class ProviderManager:
    ...
    def _check_external_providers(self):
        ...
        if directory:
            ...
            # then set the value iof the setting to an empty string (i.e None)
